<?php
    class RegistroModel extends Mysql{
        ...
        //guardar registro
        public function insertUser(string $nombre, string $ape_p, string $ape_m, int $unidad, int $cargo, int $rol, int $titulo, string $mail, string $pass, int $idUser){
            $return ='';
            ...
            if(empty($request)){
                $query_insert = "INSERT INTO usuarios(user_nom, user_ap, user_am, user_unidad, user_cargo, user_rol, user_titulo, user_mail, user_pass, user_madeBy) VALUES(?,?,?,?,?,?,?,?,?,?);";
                $arrData = array($this->strNombre, $this->strApe_p, $this->strApe_m, $this->intUnidad, $this->intCargo, $this->intRol, $this->intTitulo, $this->strMail, $this->strPass, $this->intIdUser);
